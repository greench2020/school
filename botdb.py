...
class BotDB:
    ...
    def adding_user(self, id_user, name, sec_name):
    ...
    def update(self, user_id, sgo_l, sgo_pass, ):
    ...
    def set_site_and_school(self, user_id, school):
        # self.cursor.execute(f"""UPDATE 'school_db' SET site_sgo = ? WHERE userid = ?""", (site, user_id))
        self.cursor.execute(f"""UPDATE 'school_db' SET school = ? WHERE userid = ?""", (school, user_id))
        self.conn.commit()

    def set_output_type(self, user_id, output_type):
        self.cursor.execute(f"""UPDATE 'school_db' SET output_type = ? WHERE userid = ?""", (output_type, user_id))
        self.conn.commit()

    def set_authorize(self, user_id, value):
    ...
    def change_user(self, user, param, value):
